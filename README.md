...
In general, RAG is a way to overcome the need to constantly train and retrain a Large Language Model (LLM). The LLM is powerful, but the challenge is lies with creating it in the first place.  LLM's are neural networks with 100's of billions of nodes with each node inter-connected to the others. Once trained, the models are static and work well for Generative AI use-cases, but they are very expensive to train because all the inter-connect nodes require a lot of computation to build ( and tune). The training is so computationally expensive that updating them to keep up with new data or tuning is a costly activity. You can learn more about the challenges in this in-depth paper here:  [Efficient LLM Training](https://arxiv.org/pdf/2104.04473.pdf). .
